...
| 8b | The spike's decision (2d, provisional until M1 passes): the App setup — App registration, both installations, Terraform-managed Cloud Run token broker holding the key in Secret Manager and minting `repositories: [repo]`-scoped tokens, one bearer per implementer environment mapped to exactly one repo, `factory-token` helper (replaces the `factory-token-required` sentinel, installs itself as the git credential helper as the durable form of `gh auth setup-git`, sets the App's commit author), and the cross-repo isolation test: a repo-A session cannot obtain a repo-B token even bypassing the helper. **Does not start until M1 passes**; if M1 fails, 8b is blocked and item 14 is the one remaining compliant path — the Action variant (`spawn` backend `action` + `repository_dispatch` into a `factory-implement` workflow) is not a fallback without a human revision of 2d's exit criteria. Code: `plugins/ticket-workflow/scripts/factory-token`, `factory-isolation-test`, `skills/ticket-workflow/factory-identity.md` (claude-toolbox); `services/factory-token-broker/` (toolbox) | 2d | 8 |
