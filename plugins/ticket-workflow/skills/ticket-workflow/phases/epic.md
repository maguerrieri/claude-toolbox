...
- [ ] Every child has been spawned (roots immediately; dependents once their gate passed — Step 5), on the backend the `spawn` skill selected
- [ ] Every child is START-complete (PR open on its assigned branch, CI green, review clean, base as assigned) — or reported **stuck** with what was tried
- [ ] Every simple-path chain whose PRs are all open is registered as a native stack (or the fallback noted), and its `stack:` marker recorded (Step 6)
...
- **Keep the `Worktree:` directive; it does not fight `outcome_branch`.** START Step 3's `git worktree add … -b <branch>` finds the branch already checked out and takes the Notes' "already exists → reuse it" path: the child works on that branch in the container's own checkout, and the name is exactly the one the orchestrator assigned.
...
4. **If anything is still not done, arm the next wake and end the turn:** `send_later({"delay_minutes": <N>, "message": "EPIC <epic-id> re-check — run phases/epic.md Step 6 again"})` (session-management MCP: a self-bound one-shot Routine that delivers the message as a user turn into *this* session, survives container reclaim, and disables itself after firing). Pick `<N>` from how long children actually take — 15–30 minutes for implementation work; a wake every minute buys nothing but cost. Arm exactly one per turn (the previous one already fired to produce this turn), then **stop** — ending the turn *is* the wait. A wake that finds nothing changed re-arms silently; a wake that finds every box checked continues to the stack print below (and Step 7 if a finish flag was given).
