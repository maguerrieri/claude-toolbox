...
- **notify:** set `requested` as lazy adapter metadata on every unit

Delegate every wave to generic `spawn`, which alone owns harness selection, lazy notifier resolution, native task/session creation, launch directory, isolation, stable launch address/identifier, and launch reporting. Ticket-workflow does not resolve a parent address, read `messaging.md`, or perform an identity lookup before delegation. Generic `spawn` appends a concrete `Notify:` directive only when the selected caller/target edge supports it; otherwise it omits the directive and does not perform a discarded lookup. Do not emit a harness CLI or call native task creation directly from ticket-workflow. Record the stable address/identifier generic `spawn` reports and use the active harness's `INSPECT` operation for later reads, waits, or redirects; display names are never stable addresses.
...
- **Wake-up channel:** generic `spawn` resolves `notify: requested` for the selected edge. A concrete child `Notify:` directive is an optional state-change hint; when generic `spawn` omits it, the orchestrator uses `INSPECT` polling. Step 6 always verifies against the PRs.
