...
- On success `gh issue create` prints the new issue's URL; the trailing path segment is the number (`…/issues/57` → `57`). Return that number — it's the `<n>` every other op consumes. Verify the result carries exactly one `risk:*` label **and** that it is one of the four known classes — `gh issue view <n> -R OWNER/REPO --json labels --jq '[.labels[].name | select(startswith("risk:"))]'` must print exactly one name, and that name must be in `risk:docs` / `risk:low` / `risk:normal` / `risk:high` (a sole `risk:critical` fails this check too) — and report if it doesn't.
