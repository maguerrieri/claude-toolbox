...
#    Capture the listing FIRST and check gh's own status; only then match. A failed listing is a
#    hard error in its own right, never a miss — and never a pass.
labels=$(gh api --paginate repos/OWNER/REPO/labels --jq '.[].name') || { echo "label listing failed for OWNER/REPO"; exit 1; }
printf '%s\n' "$labels" | grep -Fxq -- "<required label>"   # once per required label; any miss → hard error, stop
