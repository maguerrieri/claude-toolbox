...
# once per required label. A here-string, not a pipeline: `grep -q` exits on the first match and
# closes the pipe, so under `pipefail` a large listing makes the writer die of SIGPIPE and the
# pipeline reports 141 even though the label IS present.
grep -Fxq -- "<required label>" <<<"$labels" || { echo "OWNER/REPO is missing required label <required label> -- provision it (see Labels below), then retry"; exit 1; }
