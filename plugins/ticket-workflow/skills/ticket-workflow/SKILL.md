...
Report: PR URL, a 1–2 sentence summary, which self-review ran for each of the two passes (`/code-review high` or the manual read) and any self-review finding held at the cap, whether the review bot had non-trivial comments and how they were handled — the round count, and if the cap was hit, the `Review rounds:` line and that more rounds are theirs to grant (raise the line's cap, or say "one more round" in this session) — and that `/finish-ticket <id>` is the next step after the user's review.
