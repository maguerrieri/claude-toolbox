...
- The base hasn't moved under you: `git fetch origin <base_branch>`, then compare `git merge-base origin/<base_branch> HEAD` with `git rev-parse origin/<base_branch>` — if they differ, the base advanced (a stacked parent was restacked, or the default branch moved), so `git rebase origin/<base_branch>` first (and push with `--force-with-lease` if this branch is already on origin), or the PR's diff will carry the base's commits and a native stack containing it stops being linear.
