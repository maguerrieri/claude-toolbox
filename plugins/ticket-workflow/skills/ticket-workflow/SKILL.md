...
- **The owner's own finish request, in a session** — they invoke `/finish-ticket`, or ask to merge in their own words, there, including after attaching to a background session. That session holds the grant for what they named. When what they named is a PR a live child of this session owns, clear that child (below) rather than merging it here: the child owns its branch and worktree. EPIC Step 7, which lands a stack itself, is the exception.
