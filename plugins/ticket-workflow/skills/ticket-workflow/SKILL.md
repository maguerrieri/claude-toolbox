...
- [ ] Final self-review ran on the final diff before hand-back (Step 8, always — not only when the diff grew), recorded in the same section: neither its `Before the PR:` nor its `Final diff:` line reads `pending`
