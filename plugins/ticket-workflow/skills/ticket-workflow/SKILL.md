...
**Session identity.** Every snippet that reads or writes the marker keys it on `sid="${CLAUDE_CODE_SESSION_ID:-${CLAUDE_SESSION_ID:-}}"`. The first is set by the harness in every Bash call (Claude Code 2.1.132+) to this session's own id, the same `session_id` the hooks read. Each session sets its own, so a child never sees its parent's, and it needs no hook. The second is this plugin's SessionStart export, the fallback for older CLIs. It is an ordinary exported variable, and a `claude --bg` child launched from the Bash tool would inherit it: a child on an older CLI whose own hook didn't run would write its self-pin into the parent's marker. So every local spawn edge strips it (`env -u CLAUDE_SESSION_ID claude --bg …`), and such a child has no id and skips its pin instead. No id at all (an older CLI whose hook didn't run) → skip the marker write and let the charter govern from context. A **teleported** session (`claude --teleport`) has no marker on its new machine, since the marker stayed where the session ran, and a teleport launch doesn't deliver SessionStart output (traced in Claude Code 2.1.282). So nothing re-attaches the charter on its own: re-pin with `/role <role>` after teleporting.
...
- **Stripped identity** — *local backend only*: every launch goes through `env -u CLAUDE_SESSION_ID`, so a sibling whose own SessionStart hook didn't run can't write its `Role: implementer` self-pin into your marker (Session roles: *Session identity*). Don't drop it when hand-writing a launch.
