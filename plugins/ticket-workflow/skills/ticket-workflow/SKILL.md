...
- **Wake-up channel (default on):** read `mailbox.md`, arm your own mailbox, and put both a `Notify:` (your mailbox) and a `Mailbox:` (the sibling's own, keyed by its branch) directive in each briefing — siblings ping `pushed:`/`done:`/`blocked:`/`filed:` instead of being purely polled, and can be poked back. The PR/tracker stays the durable record.
...
- **Wake-up channel (default on** — it's what makes the Step 6 poll cheap): read `mailbox.md`, arm the orchestrator's own mailbox, and pass every child both `Notify: <orchestrator mailbox>` and its own `Mailbox:` (keyed by its assigned branch) — children ping `pushed:`/`done:`/`blocked:`/`filed:` instead of being purely polled, the orchestrator can redirect a child mid-run, and siblings can ping each other via derived keys (e.g. a parent telling its stacked dependent it restacked). Pings are hints; Step 6 still verifies against the PRs.
