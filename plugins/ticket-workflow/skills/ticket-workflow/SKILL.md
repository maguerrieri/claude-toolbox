...
- an **implementer** runs its own FINISH, gate included (a gate failure still stops it), and pings `merged: #<pr>` or `blocked: <why>`. It passes the grant to no one. A PR clearance covers the PR only once **no unmerged PR sits below it**: its base isn't the head branch of a PR that's still open. Step 2's base check then confirms the base is the expected one. Open PRs based on *its* branch don't block it, because Step 2 asks their owners to restack. A layer still based on its parent waits until the parent has merged and it has restacked. Like Step 7, the grant covers reviewed work, not findings held at the round cap. So for a PR with an unmerged PR below it, or one whose `Review rounds:` line shows `<m>` above 0 or whose `## Self-review` section has an `agree, held at the round cap` line, the implementer replies `blocked: <why>` without merging;
...
**A clearance is a workflow instruction, not approval of a permission prompt, so expect it to be refused.** Auto mode's classifier blocks "merging a pull request no human has approved" by default, and what clears that block is the user's own approval, stated in that session. A relayed grant is not that approval, and live runs (#159) saw it refused at both ends. A child's classifier treated its spawner's message loosening the merge hold as instruction poisoning, and a spawner's classifier refused to launch a session carrying merge authority, even with the owner's instruction quoted in the briefing. So treat a refusal as the expected outcome of every relay, not as an error, and never launch a session to carry a grant: no briefing carries one. A declined clearance or a blocked merge leaves that PR **`ready; needs the owner`**. Report it (`blocked: merge needs the owner`, with Step 2's block fallbacks rather than a re-clearance, which would only repeat the blocked attempt) and stop there. Don't retry it, reword it, or route it through another session: asking a peer to do what was blocked for you, or doing yourself what was blocked for a peer, launders the block. The owner lands it at the point of merge. For unattended merges they can opt in once, by adding a narrow allow rule such as `Bash(gh pr merge:*)` or an `autoMode.allow` entry to their own settings. A narrow allow rule resolves before the classifier runs; auto mode drops only broad ones, per the permission-modes docs. A session never adds or edits a permission rule to get past a block.
...
- a standing **permission rule** allowing `gh pr merge` (e.g. in the project's `.claude/settings.json`), added by the user themself (never by a session; see the FINISH intro), then re-run;
