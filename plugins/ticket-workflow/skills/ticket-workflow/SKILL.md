...
Do Step 0's **profile** selection and read its `SPAWN_CAP` — the safety cap appended to every sibling's briefing so background sessions can't over-reach (the `default` profile: implement + test, then stop at a reviewed PR and report — no prod deploy or merge unless a human steering the session asks for it mid-run — plus a trailing `Budget: rounds=5` review-round cap). Compose each briefing by appending that cap to the per-issue briefing (just the cap alone if there's no per-issue text). **Exactly one `Budget:` line rides per briefing:** a briefing that already carries its own `Budget: rounds=<n>` — in the per-issue text or in a shared "for all of these…" briefing (Step 1), raised for changes you know are risky — keeps it and drops the cap's; START Step 1 defines precedence for a directive, not for duplicates, so check the *composed* text, not just the per-issue part. This cap is the ticket layer's own bound — generic `spawn` adds none.
