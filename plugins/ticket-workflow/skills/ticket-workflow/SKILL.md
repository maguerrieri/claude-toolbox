...
- **The owner's own finish request, in a session** — they invoke `/finish-ticket`, or ask to merge in their own words, there, including after attaching to a background session. That session holds the grant for what they named. When what they named is a PR a live child of this session owns over a local `Notify:` edge, clear that child (below) rather than merging it here: the child owns its branch and worktree. EPIC Step 7 does the same for a whole stack, one layer at a time. A cloud child can't be cleared (no channel reaches it), so once its PR is ready by EPIC Step 7's test (no unmerged PR below it, and a dependent's `restacked:` posted), run FINISH here the way EPIC Step 7 does on cloud: check the child's revision out before Step 1's gate, and skip Steps 3–4's local cleanup. Or the owner attaches to the child.
...
- a standing **permission rule** allowing `gh pr merge` in the user's own settings (`~/.claude/settings.json`, or the project's `.claude/settings.local.json`), added by the user themself, never by a session (the FINISH intro has the safer `autoMode.allow` form), then re-run;
