...
On the cloud backend the same unit goes to `create_session` with the prompt as `prompt`, the name as `title`, and the repo passed explicitly as `source_url` (plus `source_revision` when the briefing pins a base branch) — see the `spawn` skill's `backends/cloud.md`, including its slash-command caveat: when the plugin isn't installed in the target environment, a prompt that *begins* with `/start-ticket` is rejected before the model runs, so open it with prose naming the skill file instead (EPIC Step 5 spells out the form).
