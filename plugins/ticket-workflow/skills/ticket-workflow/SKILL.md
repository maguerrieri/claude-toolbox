...
- **The owner's own finish request, in a session** — they invoke `/finish-ticket`, or ask to merge in their own words, there, including after attaching to a background session. That session holds the grant for what they named. When what they named is a PR a live child of this session owns over a local `Notify:` edge, clear that child (below) rather than merging it here: the child owns its branch and worktree. EPIC Step 7, which lands a stack itself, is the exception. A cloud child can't be cleared (no channel reaches it), so run FINISH here the way EPIC Step 7 does on cloud: check the child's revision out before Step 1's gate, and skip Steps 3–4's local cleanup. Or the owner attaches to the child.
...
- an **implementer** runs its own FINISH, gate included (a gate failure still stops it), and pings `merged: #<pr>` or `blocked: <why>`. It passes the grant to no one. A PR clearance covers only an **unstacked** PR, one based on the default branch with no open PR based on its branch. A stacked layer lands through its coordinator's EPIC Step 7, or through the owner. Like Step 7, the grant covers reviewed work, not findings held at the round cap. So for a stacked PR, or one whose `Review rounds:` line shows `<m>` above 0 or whose `## Self-review` section has an `agree, held at the round cap` line, the implementer replies `blocked: <why>` without merging;
