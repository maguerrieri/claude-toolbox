...
**Arm mailbox (if directed).** If the briefing carries a `Mailbox:` and/or `Notify:` directive (the cross-session wake-up channel spawn edges carry by default), read `mailbox.md` now (read-on-demand, like a tracker/profile) and follow it: arm a persistent Monitor on your `Mailbox:` path, and ping the `Notify:` path at the events it lists (`pushed:`, `done:`, `blocked:`). No directive → an edge that opted out (or a pre-mailbox spawner); nothing to arm.
...
- **Wake-up channel (default on):** read `mailbox.md`, arm your own mailbox, and put both a `Notify:` (your mailbox) and a `Mailbox:` (the sibling's own, keyed by its branch) directive in each briefing — siblings ping `pushed:`/`done:`/`blocked:` instead of being purely polled, and can be poked back. The PR/tracker stays the durable record.
...
- **Wake-up channel (default on** — it's what makes the Step 6 poll cheap): read `mailbox.md`, arm the orchestrator's own mailbox, and pass every child both `Notify: <orchestrator mailbox>` and its own `Mailbox:` (keyed by its assigned branch) — children ping `pushed:`/`done:`/`blocked:` instead of being purely polled, the orchestrator can redirect a child mid-run, and siblings can ping each other via derived keys (e.g. a parent telling its stacked dependent it restacked). Pings are hints; Step 6 still verifies against the PRs.
