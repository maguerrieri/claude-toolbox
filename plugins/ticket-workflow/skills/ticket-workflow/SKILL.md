...
- **EPIC** — expand an epic into its child tickets, run each through START **dependency-aware** (parallel where independent, stacked where one child depends on another), then aggregate the resulting **stack of PRs**, review their combined diff (routing findings back to the owning children), and hand it back — optionally finishing them.
