...
> **EPIC is local-backend only for now.** The command above, the durable launch dir, the wake-up channel, and the `claude agents` inspection in Step 6 all assume the `spawn` skill's local backend. If `spawn`'s step 3 would select **cloud** (`$CLAUDE_CODE_REMOTE_SESSION_ID` set), **stop and say so** rather than running Step 5 — a `claude --bg` child of a cloud container dies with it, and the orchestrator itself is reclaimed while it waits. Cloud EPIC needs its own pass (deterministic branches via `outcome_branch`, a re-woken rather than long-lived orchestrator, no `Notify:`); SPAWN Step 3 already delegates per backend and is the pattern to follow.
