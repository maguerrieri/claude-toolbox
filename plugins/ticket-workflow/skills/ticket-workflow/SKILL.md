...
- **(a) Normal — `<branch>` does not exist yet:** create the worktree, enter it, then init submodules:
  ...
  git worktree add <worktree_dir>/<branch> -b <branch> "$cut_from"
...
```bash
git push --force-with-lease=<branch>:$(git rev-parse origin/<branch>) origin HEAD:refs/heads/<branch>
```
...
- **Commit-message + merge-marker scan.** Rebase-merge lands the branch's commit subjects *verbatim* into the base branch's permanent history, so vet them — and catch any "not actually ready" signal that slipped past review. Inspect the commits and PR metadata (`gh pr view <pr> -R <owner>/<repo> --json headRefOid,commits,title,body,isDraft,comments,reviews`) and the lines this PR adds (`gh pr diff <pr> -R <owner>/<repo>`), checking:
...
```bash
gh pr list --state open --base <branch> -R <owner>/<repo> -L 500 \
