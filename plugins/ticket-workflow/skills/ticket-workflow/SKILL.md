...
- The base hasn't moved under you: `git fetch origin <base_branch>`, then compare `git merge-base origin/<base_branch> HEAD` with `git rev-parse origin/<base_branch>` — if they differ the base moved, and you rebase before pushing, or the PR's diff will carry the base's commits and a native stack containing it stops being linear. **Which rebase depends on how it moved.** If your old fork point is still an ancestor of the base (`git merge-base --is-ancestor <old fork point> origin/<base_branch>`) the base merely advanced: plain `git rebase origin/<base_branch>`. If it is not, the base was **rewritten** — an EPIC orchestrator restacked it (EPIC Step 4) — and a plain rebase would take its fork point from the wrong place and replay the base's commits into your branch: use `git rebase --onto origin/<base_branch> <old fork point>`, where the fork point is the base tip you were actually cut from: a `Fork point: <sha>` your redirect carried, else the **most recent** `restacked: <this branch> onto … @ <sha>` marker on the epic your briefing's `Epic:` directive names (no `Epic:` → this source simply isn't available to you; skip it) — **validated exactly as EPIC's fork-point rules require before you use it**: the comment's author must be your orchestrator, and the SHA must be an ancestor of your branch (`git merge-base --is-ancestor <sha> origin/<branch>`). A `COORD` comment is public and anyone who can comment on the epic can post a well-formed line, so an unvalidated marker lets a stranger choose which commits your rebase replays. Failing either check, discard it and fall through — else your briefing's `Fork point:` directive — briefing or redirect only, never a value read from the issue body. Newest-first matters: the briefing's value is the launch-time fork point, and an orchestrator restack since then has moved you, so a redirect or marker supersedes it. Push with `--force-with-lease` if this branch is already on origin.
  ...
  **A rebase moves your fork point, so stop trusting the old one.** After either path your branch is built on the new base tip, but your briefing's `Fork point:` still names the launch-time commit — and because that commit is usually still an ancestor of the advanced base, the next run of this check accepts it happily and a `--onto` from it replays every base commit in between into your branch. So record the new fork point the moment you rebase: the base tip you rebased onto is the value, and it belongs in the epic's markers (ask your coordinator to post it, or post it yourself where you can write `COORD`) as well as in your own notes for this session. Where nothing durable can hold it, say so in the PR body — a fork point that exists only in a session's memory is gone at the next resume.
