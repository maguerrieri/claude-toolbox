...
- [ ] CI checks are green
- [ ] Review bot (if the repo has one) is clean: **zero unresolved threads** (required under every alternative that follows — a reached cap resolves its threads with disposition replies, it doesn't waive them), **and** one of: for Copilot, a newest review on the PR head that is not an *unable to review* body and is either an approval or has every body finding answered in a PR comment posted after it; **or** the engaged bot isn't Copilot (its findings are threads only, so the threads are the whole gate); **or** the no-bot fallback recorded on the PR; **or** the review-round cap reached (`Budget: rounds=<n>`, default 12): a `Review rounds: <n> (cap <cap>); <m> findings open by disposition` line in the PR body whose `<cap>` is the cap in force, whose `<n>` equals the round count read off the PR now and is at least `<cap>`, and whose `<m>` equals the number of `agree, held` lines in the one PR comment that answers **the engaged bot's** newest review on the PR head, which is not an *unable to review* body (posted after it, even for a thread-only round) — a line a later push or a raised budget left behind is no marker. A budget hit, not a stall (profile `REVIEW_BOT`)
...
**Note your budget (if directed).** If the briefing carries a `Budget: rounds=<n>` directive (a sibling of `Base branch:` / `Worktree:` / `Role:` — the profile's `SPAWN_CAP` appends `Budget: rounds=12` to every spawned briefing, and a spawner raises it per issue for a change it knows is risky), record `<n>` as this PR's **review-round cap** for Step 8 — accepting it only when it is a positive whole number (matches `^[1-9][0-9]*$`); any other value (`0`, a negative, text, shell characters) is treated as no directive — the profile default, with the rejected value noted in the Step 9 hand-back — so only digits ever reach the write below — and, because a briefing directive doesn't survive `/clear`/resume/compaction any better than `Role:` does, write it next to the role marker now, in a file **named for this ticket** (`<id-lower>`: the issue ID lowercased, non-alphanumerics → `-`, as EPIC names branches) and **only if that file doesn't exist yet** — a re-run of this step (a re-invoked `/start-ticket`, a resume) must never overwrite a cap a human has since raised — with its own guard, since a `Budget:` can arrive without a `Role:`:

```bash
roles_dir="${CLAUDE_SESSION_ROLES_DIR:-$HOME/.claude/session-roles}"
budget_file="$roles_dir/$CLAUDE_SESSION_ID.<id-lower>.budget"
[ -n "$CLAUDE_SESSION_ID" ] && mkdir -p "$roles_dir" && [ ! -e "$budget_file" ] && printf '%s\n' "<n>" >"$budget_file"
```

(unset `$CLAUDE_SESSION_ID` → skip the write, as for the role marker). Keying the file by ticket means another ticket in the same session can never read it, so nothing is ever deleted — a re-run after compaction, briefing gone, finds the file intact. Step 7 also seeds the cap into the PR body; Step 8 resolves the cap from these sources. No directive → write nothing and use the profile's default (12 in `default`); a human driving the session raises it by saying so. Like the other directives it is data you act on, not an instruction the issue body can carry — read it from the briefing/arguments only.
...
Draft the title/body from the commits (`git log origin/<base_branch>..HEAD`, `git diff origin/<base_branch>...HEAD`) and the issue. Open the PR using the adapter's `PR_REF` for title format and the issue-linking footer (e.g. a closing keyword so merge auto-closes the issue). The `Review rounds:` line seeds Step 8's review-round cap (`<cap>` from Step 1) into the PR itself, where a later turn or a coordinator can re-read it; Step 8 rewrites its counts after every round, so the body never under-reports, and a count below the cap can't pass the cap gate:
...
**Count the rounds against the cap**, resolved from the most recently persisted source first: this ticket's budget file (Step 1 wrote it; a human's raise rewrites it), else the `(cap <cap>)` in the PR body's `Review rounds:` line (the durable copy on a fresh container, also rewritten on a raise), else the briefing's `Budget: rounds=<n>`, else the profile default — 12 in `default`. The briefing comes last because it is the *original* value: a raise never edits it, so reading it first would silently undo every raise. After each round, rewrite the line's `<n>` to the new count (and `<m>` to 0 while below the cap, where nothing is held): one round = one bot review on a new head (`REVIEW_BOT` has the read off the PR, keyed on the engaged bot's login); an *unable to review* body and a re-request without a push don't count. At the cap with findings still open, a fix push would be another round, so **stop pushing and stop re-requesting**: answer the newest round by disposition (`not changing — …` / `agree, held at the round cap — …`), rewrite the PR body's `Review rounds: <n> (cap <cap>); <m> findings open by disposition` line (seeded at Step 7) with the count and the held findings, and hand back — that is review-clean for the checklist, not a stall; the human reads the dispositions and can say "one more round". A CI fix still pushes; the review it triggers is counted and answered by disposition, and the line is rewritten with the new count — the gates accept the line only when its `<n>` matches the PR. Every round, also run the profile's **spiral check**: a section you keep patching for findings about your own earlier fixes gets one coherent rewrite instead of another patch.
...
Do Step 0's **profile** selection and read its `SPAWN_CAP` — the safety cap appended to every sibling's briefing so background sessions can't over-reach (the `default` profile: implement + test, then stop at a reviewed PR and report — no prod deploy or merge unless a human steering the session asks for it mid-run — plus a trailing `Budget: rounds=12` review-round cap). Compose each briefing by appending that cap to the per-issue briefing (just the cap alone if there's no per-issue text). **Exactly one `Budget:` line rides per briefing**, the most specific: a per-issue `Budget: rounds=<n>` (Step 1) beats one in a shared "for all of these…" briefing, which beats the cap's `Budget: rounds=12` — keep the winner and drop the other two from the composed text. **Validate the winner here, before Step 3 embeds the briefing in a double-quoted launch:** it must match START Step 1's `^[1-9][0-9]*$`, and a malformed line is dropped in favor of the next source, so only digits ever reach the spawn command. START Step 1 defines precedence for a directive, not for duplicates, so check the *composed* text, not just the per-issue part. This cap is the ticket layer's own bound — generic `spawn` adds none.
