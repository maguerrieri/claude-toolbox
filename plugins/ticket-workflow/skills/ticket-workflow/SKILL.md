...
- **The owner's own finish request, in a session** — they invoke `/finish-ticket`, or ask to merge in their own words, there, including after attaching to a background session. That session holds the grant for what they named. When what they named is a PR a live child of this session owns over a local `Notify:` edge, clear that child (below) rather than merging it here: the child owns its branch and worktree. EPIC Step 7, which lands a stack itself, is the exception. A cloud child can't be cleared (no channel reaches it), so run FINISH here instead: its branch never existed in this clone, so skip Steps 3–4's local cleanup, as EPIC Step 7 does on cloud. Or the owner attaches to the child.
