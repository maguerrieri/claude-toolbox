...
- [ ] Self-review ran on the full diff before the first push — or, for a branch already pushed without one (a resume, a re-brief, a re-spawn), once on the current full diff — (Step 7: `/code-review high`, or a manual adversarial read where that skill isn't available), and the PR body's `## Self-review` section names which ran and gives one line per finding left unfixed
- [ ] Final self-review ran on the final diff before hand-back (Step 8, always — not only when the diff grew), recorded in the same section: it has both a `Before the PR:` and a `Final diff:` line (a missing line counts as not run), and neither reads `pending`
...
**Self-review first — once, before the first push.** With tests passing, run `/code-review high origin/<base_branch>...HEAD` — pass the range explicitly: with no target the skill diffs against the branch's upstream, which after a push (or on a branch that was already on origin) is the branch itself, and reviews nothing — and fix what it finds, committing the fixes like any other. Fixes land after Step 6, so re-run the tests and re-check `DOCS` on whatever they touch before pushing. A bot reviews one head at a time and surfaces its findings one round per push; a single high-effort pass over the whole diff up front catches most of them before the first round (on #136, one `/code-review` pass in round 13 found issues that twelve Copilot rounds had missed). For each finding you decide not to fix, write one line in the PR body's `## Self-review` section (the template below), in the profile's disposition-line shape: `<anchor> — not changing — <why>`, where `<anchor>` is the finding's `path:line` or, without one, a short title. If the `code-review` skill isn't available in this session (it isn't in the skill list, or invoking it fails), do a **manual adversarial read** of the full diff instead: read it as a reviewer hunting for bugs, contradictions, and stale cross-references, not as its author. Either way, the section names which one ran. The PR doesn't exist yet, so hold the result (which ran, the counts, the unfixed lines) and write it into the body when `gh pr create` runs below, as the `Before the PR:` line. Where the section has to be added to an existing PR before this pass has run on it, seed that line as `Before the PR: pending` and replace it once the pass runs; the gates read `pending` on either line as not yet run. When the PR already exists, write it with `gh pr edit <pr> --body-file <file>`: read the body with `gh pr view <pr> --json body -q .body`, then replace the `## Self-review` section, or insert it when there is none: just above the `Review rounds:` line, or, where the body has no such line (a capless profile), right after the `## Test plan` section. This pass is not a review round: it never counts toward Step 8's cap. A branch already pushed without this pass (a resume or re-brief of a PR opened before this step existed, a re-spawn from a pushed branch) runs it once on the current full diff at its next chance, with the same range, instead of skipping it. If that PR has already reached its round cap (Step 8), don't push the fixes: hold each one as `<anchor> — agree, held at the round cap — <the fix>` in the section, as Step 8 does at the cap, and name them in the Step 9 hand-back.
...
## Self-review
Before the PR: <`/code-review high` or manual adversarial read>: <n> findings, <k> fixed
Final diff: pending
...
**Final self-review, always, before hand-back.** Once the loop above is done (review-clean by the checklist's gate, CI green), run Step 7's self-review once more on the final diff: same explicit `origin/<base_branch>...HEAD` range, same fallback. This runs every time, not only when the diff grew; a small late change can still break things. It runs once per hand-back, not every round: high effort is expensive, and cost is why the cap exists. Like Step 7's pass it never counts toward the cap. Record it in the `## Self-review` section with the same `gh pr edit` read-and-rewrite as Step 7: replace the `Final diff: pending` line with `Final diff: <which ran> on <head sha>: <n> findings, <k> fixed`, naming the head the pass reviewed. That line is the pass's durable record, which the checklist and EPIC's START-complete gate read, so a pending line keeps the PR from counting as START-complete. Handle its findings like any other: fix, or `<anchor> — not changing — <why>` in that section. Re-run the tests and re-check `DOCS` on whatever a fix touches before pushing it, as in Step 7. The fix is then an ordinary push that goes back through this loop — below the cap it goes out and the bot review it triggers counts as a round — but the final pass itself doesn't run again for it. So the recorded head can trail the PR head, after a fix from this pass or a later restack or conflict merge, and that is by design: the line records that the pass ran and what it covered, not that it is fresh. Later changes are covered by the bot review and CI they trigger, and a human who wants one more pass can ask for it. At the cap the fix is held instead, recorded as `<anchor> — agree, held at the round cap — <the fix>` in that section. If the cap is later raised and the fix pushes, rewrite that line to `<anchor> — fixed in <sha> — …` with the same `gh pr edit`, so a stale held line doesn't keep blocking EPIC's finish gate. Held self-review findings are not part of the `Review rounds:` line's `<m>` (that counts the bot round's disposition comment only), so Step 9 names them, and EPIC's finish gate stops on the held line itself.
