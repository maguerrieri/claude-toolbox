...
- **Dependents found, no `gh stack`, or a fan-out at any level** (more than one child means the dependency component isn't a simple path): merge as below, then **restack each dependent** per EPIC Step 7's unregistered rule (retarget to `<base_branch>`, then rebase **`--onto origin/<base_branch>` from that child's recorded fork point** — *not* a plain `git rebase`, whose implicit merge-base replays the parent's pre-merge commits and recreates the parent's diff in the child, which is the exact corruption that rule exists to prevent — push, re-watch CI) before handing back — a merged parent with un-restacked children is the ad-hoc failure the stacked-PR rules exist for.
