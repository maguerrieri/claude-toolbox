...
Invoking START's autonomous review cycle, or asking to **"cycle with Copilot" / "review until convergence"** on an existing PR, requests the same scoped loop: evaluate feedback, make in-scope fixes, test, commit/push, reply/resolve, and request re-review. When supported and permitted by the host, this includes recurring product-native follow-ups while the reviewer or CI is pending. An existing-PR review request enters START Step 8; it does not restart implementation or authorize unrelated work.
