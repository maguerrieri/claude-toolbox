...
```
   ...
   **A refused `--finish` launch.** Auto mode can refuse to launch a session that carries merge authority, even when the owner asked for it: on 2026-09-25 it refused a launch whose briefing quoted the owner's merge instruction (#159). If a launch carrying `--finish` (or "merge when green") is refused, don't retry it as-is or reword it. Launch once more with **every** finish form stripped from the arguments (`--finish`, "merge when green", "and finish them"), since `/start-epic` treats each as the flag. That is the plain `/spawn-epic`, which carries no merge authority. Say so in the hand-back. The epic then stops at a reviewed stack, and the owner lands it by attaching to the orchestrator and saying finish (EPIC Step 7 then clears it layer by layer) or by merging each unstacked PR themself. If that launch is refused too, stop: print the local launch command above, with the owner's original arguments, for them to run in their own terminal.
