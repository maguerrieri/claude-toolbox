...
**Invoke the `ticket-workflow` skill now via the Skill tool** and run its **EPIC** phase. The skill's EPIC index requires reading `phases/epic.md` completely through `RESOURCES`; that phase resource is authoritative. Don't restate the steps here (that just lets the command and phase drift). Parse "$ARGUMENTS" as:
