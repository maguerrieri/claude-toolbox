...
Then do the skill's **Step 0** (select tracker + profile) and run the EPIC cycle exactly as written in the skill's `phases/epic.md` (Steps 1–7, Step 6b's integration review included, which the skill's EPIC index paragraph tells you to read) — that's the authoritative flow. On the cloud backend the cycle spans several turns by design — each poll ends behind a `send_later` wake-up (EPIC Step 6); keep the completion criteria open until every box is checked.
