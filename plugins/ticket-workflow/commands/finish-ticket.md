...
This assumes the PR meets START's latest-head review/feedback and applicable-CI gates, any recurring review follow-up is stopped, and the user has reviewed. First do the skill's **Step 0** to select the tracker adapter, then run the FINISH cycle: smoke test → rebase-merge → clean up worktree/branch → close the issue → record what to watch for.
