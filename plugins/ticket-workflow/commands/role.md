...
`/role <role>` in the new or teleported session. An in-process subagent
can't pin at all: it shares its parent's session, so the PreToolUse hook
denies its marker write.
