...
while IFS= read -r read_path; do
	assert "spec 1a example: context_reads path exists: $read_path" test -e "$repo/$read_path"
...
printf '\n## Evidence\n```json\n%s\n```\n' "$filled_block" >>"$tmp"
refute "assembled PR body with two blocks fails the one-block rule" test "$(count_evidence_headings "$tmp")" -eq 1
