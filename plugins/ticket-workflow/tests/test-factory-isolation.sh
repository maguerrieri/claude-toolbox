...
#      exists to catch) fails on path 1.
...
assert "well-behaved broker: reports the foreign 403 as no token" bash -c "printf '%s' \"\$1\" | grep -q 'yielded no token (HTTP 403)'" _ "$out"
assert "well-behaved broker: helper replay refuses" bash -c "printf '%s' \"\$1\" | grep -q 'helper replayed with --repo Acme/Repo-B refuses'" _ "$out"
