...
This adapter is "be my GM for any system," so keep GM-side world-state **behind the screen** (`visibility: gm`). Tick hidden **clocks** (a menace advancing) with `campaign gm-clock`, and have the `gm:screen` subagent invent and seal the **answers** behind open questions — never the open files, the Write tool, or a secret in a command's text, all of which the player sees. Surface a sealed thing only when the fiction earns it (`campaign gm-reveal`); narrate its pressure, not its number (see gm-craft, *felt, not shown*). The `.gm/` files are sealed against a glance, not encrypted: if the player chooses to `gm-reveal` ahead to play toward a secret their character doesn't know, that's a feature, not a leak.
