...
- **Sealed at rest.** Every file under `.gm/` except the screen's own `.gitignore` and `.lock` (and a draft, below) is a sealed regular file; the sweep below seals any plaintext, hidden files included.
- **Symlinks: upkeep never writes through one.** gm's automatic upkeep (the sweep, the `.gitignore` and `.lock`) runs unprompted from hooks and checkpoints, so it never writes outside the campaign. Inside a real `.gm/`, a symlinked *file* is swapped for a local copy (a secret for a sealed copy, the `.gitignore` or `.lock` for a plain one), and its target is left alone. A symlinked *directory*, `.gm` itself included, isn't followed: the screen guards against a glance, not a filesystem rearranged on purpose. The one such link that would matter by accident, a symlinked `.gm/forge/` or `.gm/inbox/` routing drafts elsewhere, makes `campaign gm-init` refuse, so `gm:screen` stops before it drafts. And a symlink is never a draft: `gm-init` and the sweep drop any link in the draft dirs at once, and `gm-seal --from` / `forge harvest --consume` refuse to consume one (they'd read the target and delete only the link). An explicit `gm-*` command still writes wherever the player's filesystem points.
