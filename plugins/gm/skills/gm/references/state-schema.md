...
- **Sealed at rest.** Every file under `.gm/` except the screen's own `.gitignore` (and a draft, below) is a sealed regular file; the sweep below seals any plaintext, hidden files included. Writers serialize on a lock held on the `.gm` directory itself, so there's no lock file.
- **Symlinks: upkeep never writes through one.** gm's automatic upkeep (the sweep, the `.gitignore`) runs unprompted from hooks and checkpoints, so it never writes outside the campaign. Inside a real `.gm/`, a symlinked *file* is swapped for a local copy (a secret for a sealed copy, the `.gitignore` for a plain one), and its target is left alone. A symlinked *directory*, `.gm` itself included, isn't followed: the screen guards against a glance, not a filesystem rearranged on purpose. The one such link that would matter by accident, a symlinked `.gm/forge/` or `.gm/inbox/` routing drafts elsewhere, makes `campaign gm-init` refuse, so `gm:screen` stops before it drafts. And a symlink is never a draft: `gm-init` and the sweep drop any link in the draft dirs at once, file or directory, and `gm-seal --from` / `forge harvest --consume` refuse to consume one (they'd read the target and delete only the link). An explicit `gm-*` command still writes wherever the player's filesystem points.
- Sealed means a `gm-sealed v1` header line, then the base64 of its zlib-compressed text. It's obfuscation against a glance, not encryption: Claude Code's diff of a file a Bash command changed, `cat`, an editor or `git diff` shows noise instead of the secret. `campaign gm-reveal` / `gm-list` and `roll table` read it. `state.json` is JSON once unsealed (`{"clocks": {<id>: {"filled", "segments"}}, "secrets": {<id>: <text>}}`); a table is the usual `- ` list.
- **Written only by the CLIs, and a secret's text only from inside the `gm:screen` subagent.** Clocks through `campaign gm-clock`; an answer through `gm:screen`, which drafts it with the Write tool under `.gm/inbox/` and seals it with `campaign gm-seal --from` (the draft is deleted); a sealed table through `forge harvest --sealed --consume` from a draft under `.gm/forge/`. Never the Write/Edit tools in the GM's own session, and never a secret in a command's text: both are in the player's transcript.
- **Drafts are the one plaintext exception**, and short-lived: `gm:screen` writes them under `.gm/forge/` and `.gm/inbox/` and the sealing command deletes them. `.gm/.gitignore` (written by the CLIs, and by `campaign gm-init`, which `gm:screen` runs before its first draft) keeps them out of every commit, gm's own and a host vault's alike, and a draft older than an hour is a crashed subagent's leftover that the sweep below drops.
- **Legacy plaintext is read as-is**, and sealed on the next write to it. The plugin's hooks seal it before that write can show it: a PostToolUse hook right after `campaign bind` (the first thing `/gm:play` and `/gm:new-campaign` run), plus the Stop and SessionStart hooks, seal any plaintext left under a bound campaign's `.gm/` from an older version and drop stale drafts, all outside any tool call's diff. `campaign gm-migrate <dir>` does the same on demand. Rewinding to a checkpoint from before sealing seals the restored files within the rewind. Git history from before sealing still holds the plaintext.
