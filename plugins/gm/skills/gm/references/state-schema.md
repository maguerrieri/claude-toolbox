...
persona: house           # which GM persona — a NAME or a PATH (default: house)
...
`persona` takes either form. A **bare name** is searched `<campaign>/personas/` → `$GM_PERSONA_PATH` → the plugin's own `personas/` (first match wins). A **path** — anything containing `/` or starting with `~`, `.`, or `/` — is used literally. So a voice written for just this campaign lives at `<campaign>/personas/<name>/persona.md` and needs no configuration; see persona-contract's *Where personas are found*.
...
A GM voice written for *this* campaign — `personas/<name>/persona.md`, the same shape any
persona has (see persona-contract). It's the **first** place a bare `persona:` name is looked
up, so `persona: my-voice` finds it with no environment configuration, and it travels with the
save: `campaign checkpoint` versions it alongside the rest of the campaign. Absent for most
campaigns, which just name a shipped persona.
