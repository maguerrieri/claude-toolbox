...
- **Drafts are the one plaintext exception**, and short-lived: `gm:screen` writes them under `.gm/forge/` and `.gm/inbox/` and the sealing command deletes them. `.gm/.gitignore` (written by the CLIs) keeps them and the screen's `.lock` out of every commit, gm's own and a host vault's alike, and a draft older than an hour is a crashed subagent's leftover that the sweep below drops.
- **Legacy plaintext is read as-is**, and sealed on the next write to it. The plugin's Stop and SessionStart hooks also seal any plaintext left under a bound campaign's `.gm/` from an older version, and drop stale drafts, out of any tool call's diff; `campaign gm-migrate <dir>` does the same on demand. Rewinding to a checkpoint from before sealing seals the restored files within the rewind. Git history from before sealing still holds the plaintext.
