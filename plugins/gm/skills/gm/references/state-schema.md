...
- **Sealed at rest.** Every file under `.gm/` except the screen's own `.gitignore` and `.lock` (and a draft, below) is a sealed regular file; the sweep below seals any plaintext, hidden files included, and replaces a symlink with a sealed copy of what it points to. Neither it nor the `.gitignore` upkeep follows a symlinked *directory*, `.gm` itself included (upkeep runs unprompted, from hooks and checkpoints, so it never writes outside the campaign): the screen guards against a glance, not a filesystem rearranged on purpose. The one such link that would matter by accident, a symlinked `.gm/forge/` or `.gm/inbox/` routing drafts elsewhere, makes `campaign gm-init` refuse, so `gm:screen` stops before it drafts. Sealed means a `gm-sealed v1` header line, then the base64 of its zlib-compressed text. It's obfuscation against a glance, not encryption: Claude Code's diff of a file a Bash command changed, `cat`, an editor or `git diff` shows noise instead of the secret. `campaign gm-reveal` / `gm-list` and `roll table` read it. `state.json` is JSON once unsealed (`{"clocks": {<id>: {"filled", "segments"}}, "secrets": {<id>: <text>}}`); a table is the usual `- ` list.
