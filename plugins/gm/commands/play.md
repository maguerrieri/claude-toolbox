...
First **turn on autosave**: `campaign bind <saves-dir>` (`${CLAUDE_PLUGIN_ROOT}/bin/campaign`). From then on the plugin's Stop hook appends every turn's player prompt and your narration to `<saves-dir>/log/raw/` and checkpoints it — no further action from you.
