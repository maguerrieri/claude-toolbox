...
4. **Mark the raw log wrapped:** `campaign mark-wrapped <saves-dir> log/NNNN-<title>.md`, so the next wrap starts after this one. (This session's own raw log gets its marker once this wrap turn is saved, so the wrap turn itself isn't left over as unwrapped play.)
