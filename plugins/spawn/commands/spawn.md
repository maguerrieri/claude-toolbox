...
Use the `spawn` skill. First consume optional `--harness codex|claude` and `--surface desktop|cli|cloud` values once as request-global launch metadata, rejecting duplicate or conflicting values and removing the tokens from the whole request. Then parse the cleaned "$ARGUMENTS" into one or more `(prompt, desc)` units (a single task is the common case; a `;`-separated list or "N agents to each do X" fans out). The one selected pair applies to every unit; per-unit overrides are unsupported. **Select the harness and surface** using the skill's step 3, read exactly the matching adapter path, and spawn one task per unit named `<context> <desc>` in parallel. Report the selected pair, its stable task/session identifiers, and its own inspect/open controls, then hand back without blocking.
