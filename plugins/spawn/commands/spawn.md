...
Use the `spawn` skill. Parse "$ARGUMENTS" into one or more `(prompt, desc)` units (a single task is the common case; a `;`-separated list or "N agents to each do X" fans out), spawn one `claude --bg --name "<context> <desc>" "<prompt>"` session per unit (in parallel — one Bash call each in a single message) **from a durable launch directory** (the repo's main checkout, never a disposable worktree — see the skill's step 3), report the table of spawned sessions, and hand back — don't block on them.

If a prompt contains shell metacharacters (`$`, backticks, `$(...)`), don't interpolate it raw into double quotes — the shell would expand and corrupt it. Pass it via a single-quoted heredoc into a variable instead (see the skill's step 4).
