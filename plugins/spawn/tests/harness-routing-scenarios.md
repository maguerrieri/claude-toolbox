...
| Lazy notifier resolution | `/spawn-tickets #63`, `/spawn-epic #40`, and an EPIC child wave from Codex desktop, Claude cloud, and Claude CLI | Every ticket-workflow unit sets `notify: requested` without resolving a session identity. Only generic `spawn` on a Claude-CLI-to-Claude-CLI edge resolves its spawner name/handle and appends `Notify:`; other pairs perform no `ListAgents` lookup. The thin epic launcher preserves `--harness`, `--surface`, name, `Role: epic-coordinator`, and the absence of `SPAWN_CAP` while delegating the sole launch to generic `spawn`. |
